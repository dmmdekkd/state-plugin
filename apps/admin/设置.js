...
// 统一支持 #sp设置 / #state设置 
const indexCfgReg = new RegExp(
  `^#?(?:state|sp)设置(${Object.keys(indexCfgTypeMap).join("|")}|前缀)(开启|关闭|(\\d+)秒?|\\s.+)?$`
)
...
export class Admin_Index extends plugin {
  ...
  async indexSet(e) {
    ...
    if (rawkey === "前缀") {
      // 修改状态前缀，重启生效
      if (!value) {
        return e.reply("请提供新的状态前缀，例如：#sp设置前缀 新前缀")
      }
